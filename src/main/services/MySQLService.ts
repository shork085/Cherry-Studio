...
export class MySQLService {
  ...
  public async getFormulaMaterials(formulaId: number) {
    const sql = `
     SELECT
      ...
      fm.test_quality,
      m.price,
      ROUND(fm.test_quality / SUM(fm.test_quality) OVER ( PARTITION BY  fm.group_type) * 100, 2)
      AS quality_score
  ...
  // 获得某配方id
  public async getFormulaId(formulaName: string): Promise<number | null> {
    const rows = await this.query<{ id: number }>('SELECT id FROM formulas WHERE type=? LIMIT 1', [formulaName])
