...
const Sidebar: FC = () => {
  ...
  const { sidebarIcons, updateSidebarVisibleIcons } = useSettings()
  ...
  // 确保 dataviz 显示在侧边栏（若用户设置里缺失，则自动补齐一次）
  React.useEffect(() => {
    if (!sidebarIcons.visible.includes('dataviz')) {
      const next = [...sidebarIcons.visible, 'dataviz']
      updateSidebarVisibleIcons(next as any)
    }
...
const MainMenus: FC = () => {
  ...
  const iconMap = {
    ...
    dataviz: <BarChart3 size={18} className="icon" />
  }
...
const Container = styled.div<{ $isFullscreen: boolean }>`
  ...
  min-width: var(--sidebar-width, 60px);
  height: 100vh;
