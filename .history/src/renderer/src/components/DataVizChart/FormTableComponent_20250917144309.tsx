...
// 内联可编辑文本（用于原料代码）
const InlineEditableText: React.FC<{ value: string; onSave: (val: string) => void }> = ({ value, onSave }) => {
  ...
  return isEditing ? (
    ...
    <span style={{ display: 'inline-block', width: '100%' }} onClick={() => setIsEditing(true)}>
      {value}
    </span>
  )
}
  ...
  switch (key) {
    case 'material_code':
      if (onMaterialCodeChange) {
        return <InlineEditableText value={item.material_code} onSave={(val) => onMaterialCodeChange(item, val)} />
      }
...
const FormTableComponent: React.FC<FormTableComponentProps> = ({ title, data, columns, onTestQualityChange, onMaterialCodeChange }) => {
  return (
    <FormItem>
        ...
        <FormTable>
          ...
          <tbody>
            {data.map((item, index) => (
              <tr key={index}>
                {columns.map((column) => (
                  <td key={column.key}>{renderCellContent(item, column.key, onTestQualityChange, onMaterialCodeChange)}</td>
                ))}
              </tr>
            ))}
          </tbody>
