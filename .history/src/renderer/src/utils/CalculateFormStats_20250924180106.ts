...
export interface ComputeResult {
  ...
  // equivalent weight reactive material
  meta: {
    system: SystemType
...
export function computeFormStats(
  ...
  // 组分A/B质量比(mixing ratio by weight)
  let wA = 100
  let wB = 0
  const errors: string[] = []
  ...
  // 混合后的质量分配（wt%）
  const setMassPct = mixMassPct({ wA: wA, wB: wB }, gmA.massPctByPhase, gmB.massPctByPhase)
  const setMassSolidsPct = setMassPct.binder + setMassPct.pigment

  // 混合后的体积分配（v%）
  const setVolumePct = mixVolumePct({ wA: wA, wB: wB }, gmA.volPer100gByPhase, gmB.volPer100gByPhase)
  ...
  return {
    ...
    phases: {
      A: { volumePct: round(gmA.volPctByPhase, 2) },
      B: { volumePct: round(gmB.volPctByPhase, 2) },
