...
export function computeFormStats(
  ...
  // 混合后的质量分配（wt%）
  const setMassPct = mixMassPct({ wA, wtB }, gmA.massPctByPhase, gmB.massPctByPhase)
  const setMassSolidsPct = setMassPct.binder + setMassPct.pigment

  // 混合后的体积分配（v%）
  const setVolumePct = mixVolumePct({ wA, wtB }, gmA.volPer100gByPhase, gmB.volPer100gByPhase)
