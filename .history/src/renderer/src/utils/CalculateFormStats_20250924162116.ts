...
function findMaterial(
  dict: MaterialDictEntry[],
  materialCode?: string | number | null,
  materialDesc?: string | null
): MaterialDictEntry | undefined {
  const codeStr = materialCode == null ? '' : String(materialCode).trim()
  const descStr = materialDesc == null ? '' : String(materialDesc).trim().toLowerCase()
  const byCode = codeStr
    ? dict.find((m) => (m.champ && String(m.champ).trim() === codeStr) || (m.sap && String(m.sap).trim() === codeStr))
    : undefined
  if (byCode) return byCode
  if (descStr) {
    return dict.find((m) => String(m.name || '').trim().toLowerCase() === descStr)
  }
  return undefined
}
...
function accumulatePhaseMassPct(rows: FormulaRow[], dict: MaterialDictEntry[]): GroupPhaseMassPct {
  ...
  for (const row of rows) {
    ...
    // @ts-ignore 允许行上带 material_desc 用于兜底匹配
    const m = findMaterial(dict, row.material_code, (row as any).material_desc)
...
function accumulateVolumes(rows: FormulaRow[], dict: MaterialDictEntry[]): GroupVolumes {
  // v = m / ρ，四相分别汇总
...
export function computeFormStats(
  ...
  const volumesA = accumulateVolumes(formDataA, materialDict)
  ...
  const volumesB = accumulateVolumes(formDataB, materialDict)
  ...
  const solids_wt_set = binder_wt_set_A + pigment_wt_set_A + binder_wt_set_B + pigment_wt_set_B
