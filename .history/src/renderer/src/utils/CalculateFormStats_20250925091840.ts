...
export interface ReactiveStats {
  binder_total: number // 该对象（A/B/MIX）中的成膜物总质量（按100g口径或混合后的绝对质量）
  reactive_mass: number // 其中参与反应的成膜物质量
  nonreactive_mass: number // 不参与反应的成膜物质量
  reactive_pct: number // reactive_mass / binder_total × 100
  nonreactive_pct: number // nonreactive_mass / binder_total × 100
  ratio_reactive_to_nonreactive: number | null // reactive_mass / nonreactive_mass（分母为0则null）
}
...
export function computeFormStats(
  ...
  */
  ...
  const reactA = pickReactiveBinderMass(system, gmA) // 每100g A中参与反应的binder质量
  const reactB = pickReactiveBinderMass(system, gmB) // 每100g B中参与反应的binder质量
  const statsA = toReactiveStats(gmA.binderMassPer100g, reactA)
  const statsB = toReactiveStats(gmB.binderMassPer100g, reactB)

  // 混合物中的“绝对质量”（以100g混合物为总质量口径）
  const binderSetAbs = gmA.binderMassPer100g * (wtA / 100) + gmB.binderMassPer100g * (wtB / 100)
  const reactiveSetAbs = reactA * (wtA / 100) + reactB * (wtB / 100)
  const statsMix = toReactiveStats(binderSetAbs, reactiveSetAbs)
  ...
  return {
    ...
    reactivity: {
      A: roundReactiveStats(statsA),
      B: roundReactiveStats(statsB),
      MIX: roundReactiveStats(statsMix)
    }
  }
}
...
function computeGroup(rows: ComputeInputRow[], index: ReturnType<typeof buildIndex>) {
  ...
  let reactiveBinderMass_eew = 0
  let reactiveBinderMass_ahew = 0
  let reactiveBinderMass_iew = 0
  let reactiveBinderMass_hew = 0
  let binderMass_total = 0
  ...
  for (const r of rows) {
    ...
    for (const p of phases) {
      ...
      if (p === 'binder') {
        binderMass_total += pmass
        if (eew) reactiveBinderMass_eew += pmass
        if (ahew) reactiveBinderMass_ahew += pmass
        if (iew) reactiveBinderMass_iew += pmass
        if (hew) reactiveBinderMass_hew += pmass
      }
    }
  ...
  return {
    ...
    binderMassPer100g: binderMass_total,
    reactiveBinderMass: {
      eew: reactiveBinderMass_eew,
      ahew: reactiveBinderMass_ahew,
      iew: reactiveBinderMass_iew,
      hew: reactiveBinderMass_hew
    }
  }
}
...
function emptyGroup() {
  return {
    ...
    binderMassPer100g: 0,
    reactiveBinderMass: { eew: 0, ahew: 0, iew: 0, hew: 0 }
  }
}
...
function pickReactiveBinderMass(system: SystemType, gm: ReturnType<typeof computeGroup>) {
  // 按体系选择“参与反应的 binder”口径
  if (system === '环氧') return gm.reactiveBinderMass.eew || 0
  if (system === '聚氨酯') return gm.reactiveBinderMass.hew || 0
  return 0 // 单组份不做反应性统计
}

function toReactiveStats(binderTotal: number, reactiveMass: number): ReactiveStats {
  const bt = clampPositive(binderTotal)
  const rm = clampPositive(reactiveMass)
  const nrm = bt - rm >= 0 ? bt - rm : 0
  const reactive_pct = bt > 0 ? (rm / bt) * 100 : 0
  const nonreactive_pct = bt > 0 ? (nrm / bt) * 100 : 0
  const ratio = nrm > 0 ? rm / nrm : null
  return {
    binder_total: bt,
    reactive_mass: rm,
    nonreactive_mass: nrm,
    reactive_pct,
    nonreactive_pct,
    ratio_reactive_to_nonreactive: ratio
  }
}

function roundReactiveStats(s: ReactiveStats): ReactiveStats {
  return {
    binder_total: round(s.binder_total, 4) || 0,
    reactive_mass: round(s.reactive_mass, 4) || 0,
    nonreactive_mass: round(s.nonreactive_mass, 4) || 0,
    reactive_pct: round(s.reactive_pct, 2) || 0,
    nonreactive_pct: round(s.nonreactive_pct, 2) || 0,
    ratio_reactive_to_nonreactive:
      s.ratio_reactive_to_nonreactive != null ? round(s.ratio_reactive_to_nonreactive, 4) : null
  }
}
