...
export function computeFormStats(
  ...
  */
  ...
  const absA = {
    binder: (gmA.volPer100gByPhase.binder || 0) * (wtA / 100),
    pigment: (gmA.volPer100gByPhase.pigment || 0) * (wtA / 100)
  }
  const absB = {
    binder: (gmB.volPer100gByPhase.binder || 0) * (wtB / 100),
    pigment: (gmB.volPer100gByPhase.pigment || 0) * (wtB / 100)
  }
  ...
  const reactVolA_abs = reactVolA_per100 * (wtA / 100)
  const reactVolB_abs = reactVolB_per100 * (wtB / 100)
  ...
  return {
    ...
    binderView: {
      ...
      non_react_base: non_react_base,
      non_react_hardener: non_react_hardener,
      non_react_set: non_react_set,
