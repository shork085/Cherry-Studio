...
export async function fetchWebContent(
  ...
  try {
    ...
    if (usingBrowser) {
      ...
      const promisesToRace: [Promise<string>] = [windowApiPromise as Promise<string>]

      if (httpOptions?.signal) {
        const signal = httpOptions.signal
        const abortPromise = createAbortPromise<string>(signal, windowApiPromise as Promise<string>)
