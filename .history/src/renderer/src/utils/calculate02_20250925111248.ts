...
export interface ComputeResult {
  ...
  reactivity: {
    A: {
      reactive_binder_wt_set: number // A端在set中的反应性binder质量（=质量百分比）
    ...
    MIX: {
      ...
      non_reactive_pct_in_binder: number
      ratio_reactive_to_nonreactive: number | null // reactive_set / non_reactive_set
    }
  }
}
...
export function computeFormStats(
  ...
  */
  ...
  // 先把 A/B 在 set 中的 binder 质量（=质量百分比）拆出
  ...
  // A/B 在 set 中的“反应性 binder 质量”（=质量百分比）
  ...
  // reactive% / non-reactive%（在总 binder 中）
  ...
  return {
    ...
    reactivity: {
      A: {
        reactive_binder_wt_set: round(reactive_binder_wt_set_A, 4),
        non_reactive_binder_wt_set: round(non_reactive_binder_wt_set_A, 4)
      },
      B: {
        reactive_binder_wt_set: round(reactive_binder_wt_set_B, 4),
        non_reactive_binder_wt_set: round(non_reactive_binder_wt_set_B, 4)
      },
      MIX: {
        reactive_binder_wt_set: round(reactive_binder_wt_set, 4),
        non_reactive_binder_wt_set: round(non_reactive_binder_wt_set, 4),
        total_binder_wt_set: round(total_binder_wt_set, 4),
        reactive_pct_in_binder: round(reactive_pct_in_binder, 2),
        non_reactive_pct_in_binder: round(non_reactive_pct_in_binder, 2),
        ratio_reactive_to_nonreactive:
          ratio_reactive_to_nonreactive != null ? round(ratio_reactive_to_nonreactive, 4) : null
      }
    }
  }
}
