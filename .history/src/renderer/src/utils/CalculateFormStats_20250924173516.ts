...
export function computeFormStats(
  ...
  // 组分A/B的质量比(mixing ratio by weight)
  let wtA = 100
  let wtB = 0
  const errors: string[] = []
  ...
  // 混合后的质量分配（wt%）
  const setMassPct = mixMassPct({ wtA, wtB }, gmA.massPctByPhase, gmB.massPctByPhase)
  const setMassSolidsPct = setMassPct.binder + setMassPct.pigment

  // 混合后的体积分配（v%）
  const setVolumePct = mixVolumePct({ wtA, wtB }, gmA.volPer100gByPhase, gmB.volPer100gByPhase)
