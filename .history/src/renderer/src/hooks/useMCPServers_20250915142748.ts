...
// Listen for server changes from main process (Web 环境下可能没有 electron 对象)
try {
  const electron = (window as any).electron
  const ipc = electron?.ipcRenderer
  if (ipc?.on) {
    ipc.on(IpcChannel.Mcp_ServersChanged, (_event: any, servers: MCPServer[]) => {
      store.dispatch(setMCPServers(servers))
    })

    ipc.on(IpcChannel.Mcp_AddServer, (_event: any, server: MCPServer) => {
