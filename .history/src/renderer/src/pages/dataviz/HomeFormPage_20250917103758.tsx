...
const HomeFormPage: React.FC<Props> = ({ searchKeyword = '' }) => {
  ...
  const handleTestQualityChange = async (item: TableDataItem, newValue: number) => {
    ...
    try {
      ...
      // 获取当前版本号
      const currentVersion = await window.api.dataviz.getMaterialVersion(currentFormulaId, item.material_id, item.group_type)
      
      console.log('获取到的版本号:', currentVersion)
