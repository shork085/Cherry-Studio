...
const HomeFormPage: React.FC = () => {
  ...
  // 表格列配置
  const tableColumns = [
    { key: 'material_code', label: '原料代码', width: 'auto' },
    { key: 'material_desc', label: '原料描述', width: '260px' },
    { key: 'test_quality', label: '测试质量', width: 'auto' },
    { key: 'quality_score', label: '质量分数%', width: 'auto' },
    { key: 'mix', label: '混合后%', width: 'auto' },
    { key: 'price', label: '参考价', width: 'auto' }
  ]
  ...
  const renderCellContent = (item: any, key: string) => {
    switch (key) {
      ...
      case 'mix':
        return item[key] != null && !isNaN(Number(item[key])) ? Number(item[key]).toFixed(2) : '-'
      case 'price':
        return `￥${Number(item[key]).toFixed(2)}`
      default:
        return item[key]
  ...
  return (
    <FormContainer>
      ...
      <FormContent>
        <TablesContainer>
          {tableData.map((table) => (
            <FormItem key={table.title}>
              <FormTitle>{table.title}</FormTitle>
              <TableWrapper>
                <FormTable>
                  <thead>
                    <tr>
