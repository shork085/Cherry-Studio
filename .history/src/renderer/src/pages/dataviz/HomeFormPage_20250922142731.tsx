...
const HomeFormPage: React.FC<Props> = ({ searchKeyword = '' }) => {
  ...
  // 处理菜单内部按下，阻止 mousedown 冒泡，避免全局监听提前关闭
  const handleMenuMouseDown = (e: React.MouseEvent) => {
    e.stopPropagation()
  }

  // 监听全局点击事件来关闭悬浮菜单（使用 click 而非 mousedown）
  React.useEffect(() => {
      ...
      <FormContent>
        ...
        {showSelectMenu && (
          <SelectMenuOverlay onClick={handleOutsideClick}>
            <SelectMenu $isAnimating={isMenuAnimating} onClick={handleMenuClick} onMouseDown={handleMenuMouseDown}>
