  ...
  const centerInputEl = isHome ? (
    <CenterBox>
      <RatioInput
        ...
        size="small"
        value={Number.isFinite(ratio) ? String(ratio) : ''}
        onChange={(e) => {
          const v = parseFloat(e.target.value)
          if (Number.isFinite(v)) setRatio(v)
          else if (e.target.value.trim() === '') setRatio(NaN as unknown as number)
        }}
      />
    </CenterBox>
  ) : null
    ...
    <Container>
      ...
      <Content>
        <Routes>
          ...
          <Route path="home" element={<HomeFormPage searchKeyword={search} stoichRatio={Number.isFinite(ratio) ? ratio : 1} />} />
