...
const materialDict: any[] = (window as any).require?.('../../../../resources/data/materialData.json') ?? []
...
const HomeFormPage: React.FC<Props> = ({ searchKeyword = '', stoichRatio = 1 }) => {
  ...
  const computedStats = React.useMemo(() => {
    try {
      const result = computeFormStats(
        (formDataA as any[]).map((r) => ({ material_code: r.material_code ?? r.sap, quality_score: r.quality_score })),
        (formDataB as any[]).map((r) => ({ material_code: r.material_code ?? r.sap, quality_score: r.quality_score })),
        materialDict as any,
  ...
  const bottomFormConfig = [
    ...
    },
    {
      ...
      isSmall: true,
      items: [
        { label: '组分A的质量比', value: computedStats?.view.mass_ratio_a ?? formStat[0]?.mass_ratio_a, unit: 'wt%' },
        { label: '组分B的质量比', value: computedStats?.view.mass_ratio_b ?? formStat[0]?.mass_ratio_b, unit: 'wt%' },
        { label: '组分A的体积比', value: computedStats?.view.volume_ratio_a ?? formStat[0]?.volume_ratio_a, unit: 'v%' },
        { label: '组分B的体积比', value: computedStats?.view.volume_ratio_b ?? formStat[0]?.volume_ratio_b, unit: 'v%' }
      ]
    },
    {
      title: '配方价格',
