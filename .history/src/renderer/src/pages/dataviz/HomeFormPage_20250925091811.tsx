...
const HomeFormPage: React.FC<Props> = ({ searchKeyword = '', stoichRatio = 1 }) => {
  ...
  const bottomFormConfig = [
    ...
    },
    {
      ...
      isRatioTable: true,
      ratioData: {
        headers: ['ratio reactive / non reactive binder', '%'],
        rows: [
          {
            label: 'reactive',
            values: [computedStats?.reactivity?.MIX?.reactive_pct || '-', '%']
          },
          {
            label: 'non reactive',
            values: [computedStats?.reactivity?.MIX?.nonreactive_pct || '-', '%']
          }
        ]
      }
    }
              ...
              <BottomFormItem
                ...
                {config.isSolidTable ? (
                  ...
                  <RatioTableContainer>
                    <RatioTableHeader>
                      <div></div>
                      {config.ratioData.headers.map((header, index) => (
                        <div key={index}>{header}</div>
                      ))}
                    </RatioTableHeader>
...
const RatioTableHeader = styled.div`
  display: grid;
