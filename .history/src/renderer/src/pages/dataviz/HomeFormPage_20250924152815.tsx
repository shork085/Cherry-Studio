...
import { computeAll, defaultResolver, selectBottomFormValues } from './calc'
// 注意：渲染进程别名通常是 @ 或相对路径。此处改为从根相对 resources 引用：
// 使用 window.require 以避免 TS 对 json 路径的类型报错

const materialDict = (window as any).require?.('../../../../resources/data/materialData.json') ?? []
  ...
  const computedStats = React.useMemo(() => {
    try {
      const result = computeAll(
        ...
        formDataB as any,
        materialDict as any,
        { bEquivalentKey: 'ahew', resolver: defaultResolver },
        stoichRatio || 1
      )
      const view = selectBottomFormValues(result)
      return { result, view }
    } catch (e) {
      return undefined
    }
