...
import { computeFormulaStats } from '../../utils/CalculateFormStats'
...
const HomeFormPage: React.FC<Props> = ({ searchKeyword = '', stoichRatio = 1 }) => {
  ...
  const bottomFormConfig = [
    {
      ...
      items: [
        {
          label: '组分A的比重',
          // value: computedStats?.view?.density_a ?? formStat[0]?.density_a,
          value: formStat[0]?.density_a,
          unit: 'g/cm3'
        },
        {
          label: '组分B的比重',
          // value: computedStats?.view?.density_b ?? formStat[0]?.density_b,
          value: formStat[0]?.density_b,
          unit: 'g/cm3'
        },
        {
          label: '混合物的比重',
          // value: computedStats?.view?.density_mix ?? formStat[0]?.density_mix,
          value: formStat[0]?.density_mix,
          unit: 'g/cm3'
        }
      ]
    },
    ...
    },
    {
      ...
      solidData: {
        ...
        rows: [
          {
            label: '质量固含(wt%)',
            values: [formSolidA[0]?.mass_fraction, formSolidB[0]?.mass_fraction, formSolidMix[0]?.mass_fraction],
            unit: 'wt%'
          },
          {
            label: '体积固含(v%)',
            values: [formSolidA[0]?.volume_fraction, formSolidB[0]?.volume_fraction, formSolidMix[0]?.volume_fraction],
            unit: 'wt%'
          },
          {
            label: '体积比',
            values: [formStat[0]?.volume_ratio_a, formSolidB[0]?.volume_ratio_a, formSolidMix[0]?.volume_ratio_a],
            unit: 'v%'
          },
          {
            label: 'PVC',
            values: [formSolidA[0]?.pvc, formSolidB[0]?.pvc, formSolidMix[0]?.pvc],
            unit: 'v%'
          }
        ]
      }
  ...
  React.useEffect(() => {
    try {
      ...
      bottomFormConfig.forEach((list) => {
        merged[list.title] = Object.prototype.hasOwnProperty.call(parsed, list.title) ? !!parsed[list.title] : true
      })
  ...
  // 每次 formDataA, formDataB 或 stoichRatio 变化时，重新计算
  React.useEffect(() => {
    if (formDataA.length > 0 || formDataB.length > 0) {
      const stats = computeFormulaStats(formDataA, formDataB, stoichRatio)
      setFormStat([stats]) // 保持数组结构，兼容原本 bottomFormConfig
    }
