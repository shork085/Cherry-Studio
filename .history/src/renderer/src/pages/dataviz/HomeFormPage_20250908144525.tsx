...
import db, { type FormMaterial } from '@renderer/databases'

// 初始种子数据（用于首次写入数据库以及组件初始渲染）
...
const HomeFormPage: React.FC = () => {
  ...
  const [formDataA, setFormDataA] = React.useState(initialFormDataA)

  const [formDataB, setFormDataB] = React.useState(initialFormDataB)

  // 从 Dexie 数据库加载数据；如为空则用默认数据进行一次性初始化
  React.useEffect(() => {
    const generateId = () =>
    ...
    const load = async () => {
      try {
        const total = await db.table('form_materials').count()
        if (total === 0) {
          // 初始化种子数据
          const seedA: FormMaterial[] = initialFormDataA.map((m, idx) => ({
            id: generateId(),
            component: 'A',
            name: m.name,
            desc: m.desc,
            testQuality: m.testQuality,
            qualityScore: m.qualityScore,
            mixQuality: m.mixQuality,
            price: m.price,
            order: idx
          }))
          const seedB: FormMaterial[] = initialFormDataB.map((m, idx) => ({
            id: generateId(),
            component: 'B',
            name: m.name,
            desc: m.desc,
          ...
          await db.table('form_materials').bulkAdd([...seedA, ...seedB])
        }

        const listA = await db.table('form_materials').where('component').equals('A').sortBy('order')
        const listB = await db.table('form_materials').where('component').equals('B').sortBy('order')
        ...
        // 出错时保持现有内置数据
        loggerService.withContext('HomeFormPage').error('加载表单材料失败', e as any)
      }
    }
        ...
        <BottomForm>
          ...
          <BottomFormItem>
            ...
            <FormInfo>
              ...
              <p title="14.62">14.62</p>
              <p title="RMB/kg">RMB/kg</p>
            </FormInfo>
            <FormInfo>
              <div title="配方的价格">配方的价格</div>
              <p title="16.24">16.24</p>
              <p title="RMB/kg">RMB/kg</p>
            </FormInfo>
          </BottomFormItem>
          ...
          <BottomFormItem className="smallForm">
            ...
            <FormInfo $small>
              ...
              <p title="56.05">56.05</p>
              <p title="wt%">wt%</p>
            </FormInfo>
            <FormInfo $small>
              <div title="体积固含(v%)">体积固含(v%)</div>
              <p title="19.41">19.41</p>
              <p title="wt%">wt%</p>
            </FormInfo>
            <FormInfo $small>
              <div title="组分A的体积比">组分A的体积比</div>
