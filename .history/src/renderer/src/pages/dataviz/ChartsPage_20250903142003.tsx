    ...
    ;(async () => {
      ...
      try {
        ...
        try {
          ...
          console.error('折线图初始化失败:', initError)
          return
        }
        const option = {
          xAxis: {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        // 尝试导入 ecStat，如果失败则使用备用方案
        let ecStatAvailable = false
        try {
          const ecStat = await import('echarts-stat')
          echarts.registerTransform(ecStat.default.regression)
          ecStatAvailable = true
        } catch (ecStatError) {
          console.warn('ecStat 加载失败，使用备用回归线方案:', ecStatError)
        }

        instance = echarts.init(containerRef.current)
        // 根据 ecStat 是否可用选择不同的配置
        let option
        if (ecStatAvailable) {
          // 使用 ecStat 的回归转换
          option = {
