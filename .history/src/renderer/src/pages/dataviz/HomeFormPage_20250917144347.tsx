...
const HomeFormPage: React.FC<Props> = ({ searchKeyword = '' }) => {
  ...
  const handleTestQualityChange = async (item: any, newValue: number) => {
    ...
    try {
      setIsLoading(true)
      if (!item.material_id || !item.group_type) {
        throw new Error('原料信息不完整，无法更新')
      }
      // 获取当前版本号
      const currentVersion = await window.api.dataviz.getMaterialVersion(
        currentFormulaId,
        item.material_id,
        item.group_type
      )
      ...
      // 更新测试质量
      await window.api.dataviz.updateMaterialTestQuality(
        currentFormulaId,
        item.material_id,
        item.group_type,
        newValue,
        ...
        <TablesContainer>
          {tableData.map((table) => (
            <FormTableComponent
              ...
              onMaterialCodeChange={async (item, newCode) => {
                if (!currentFormulaId) return
                try {
                  setIsLoading(true)
                  if (!item.group_type || !item.material_code) {
                    throw new Error('原料信息不完整，无法更新')
                  }
                  // 获取当前版本号（行里若已带version可直接用）
                  const currentVersion =
                    item.version ??
                    (await window.api.dataviz.getMaterialVersion(currentFormulaId, item.material_id, item.group_type))
                  if (currentVersion == null) throw new Error('无法获取原料版本信息')

                  // 调用更新原料代码的存储过程
                  await window.api.dataviz.updateMaterialCode(
                    currentFormulaId,
                    newCode,
                    item.material_code,
                    item.group_type,
                  ...
                  alert(error.message || '更新原料代码失败，请重试')
                } finally {
                  setIsLoading(false)
                }
              }}
