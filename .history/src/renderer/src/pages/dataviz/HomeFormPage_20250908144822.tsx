...
const HomeFormPage: React.FC = () => {
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        // @ts-ignore preload exposes window.api
        if (window.api?.mysql) {
          // 这里的表名请替换成你的实际表名
          const { a, b } = await window.api.mysql.queryForm({ tableA: 'form_a', tableB: 'form_b' })
          if (Array.isArray(a) && a.length) setFormDataA(a as any)
          if (Array.isArray(b) && b.length) setFormDataB(b as any)
        }
      } catch (e) {
  ...
  return (
    <FormContainer>
      {/* 可选：在顶部添加一个简单的加载按钮/配置入口，后续可改成设置页 */}
      <ButtonContainer>
        {btnList.map((btn) => (
