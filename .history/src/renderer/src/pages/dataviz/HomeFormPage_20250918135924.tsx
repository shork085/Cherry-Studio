...
const HomeFormPage: React.FC<Props> = ({ searchKeyword = '' }) => {
  ...
  const handleTestQualityChange = async (item: any, newValue: number) => {
    ...
    setFormDataA((prev) => prev.map((row) => (row.id === item.id ? { ...row, test_quality: newValue } : row)))
    setFormDataB((prev) => prev.map((row) => (row.id === item.id ? { ...row, test_quality: newValue } : row)))
    ...
    try {
      ...
      setFormDataA((prev) => prev.map((row) => (row.id === item.id ? { ...row, ...item.id } : row)))
      setFormDataB((prev) => prev.map((row) => (row.id === item.id ? { ...row, ...item.id } : row)))
    } catch (error: any) {
