...
function DataVizApp(props: React.HTMLAttributes<HTMLElement>) {
  return React.createElement('data-viz-app', props)
}
...
const DataVizPage: React.FC = () => {
  ...
  return (
    <Container>
      {loaded ? null : <LoadingOverlay>加载数据可视化...</LoadingOverlay>}
      {/* Vue 自定义元素 */}
      <div style={{ flex: 1, display: loaded ? 'block' : 'none' }}>
        <DataVizApp style={{ display: 'block', width: '100%', height: '100%' }} />
      </div>
    </Container>
  )
}
