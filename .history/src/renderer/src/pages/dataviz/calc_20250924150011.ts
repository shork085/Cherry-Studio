...
// STEP1：组分内各相质量百分比
export function computeGroupPhaseMassPct(rows: GroupRow[], dict: MaterialDictItem[], resolver: ComputeOptions['resolver']): GroupPhaseMassPct {
...
export function computeGroupVolumeAndDensity(
  ...
  let vol: Record<PhaseKey, number> = { binder: 0, pigment: 0, solvent: 0, water: 0 }
  for (const r of rows) {
    const idx = resolver ? resolver(r, dict) : defaultResolver(r, dict)
    if (idx === undefined || idx < 0) continue
    const m = dict[idx]
...
export function computeMix(
  ...
  const solids_wt_set = clampPct(
    (groupA.solidsMassPct * massA + groupB.solidsMassPct * massB) / 100 / (massA + massB) * 100
  )
...
export function computeAll(
  ...
  const stoich = computeStoichiometry(rowsA, rowsB, dict, options.resolver || defaultResolver, options.bEquivalentKey, r)
