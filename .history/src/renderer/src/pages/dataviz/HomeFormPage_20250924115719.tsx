  ...
  const bottomFormConfig = [
    ...
    },
    {
      ...
      solidData: {
        ...
        rows: [
          { label: '质量固含(wt%)', values: [formSolidA[0]?.mass_fraction, formSolidB[0]?.mass_fraction, formSolidMix[0]?.mass_fraction], unit: 'wt%' },
          { label: '体积固含(v%)', values: [formSolidA[0]?.volume_fraction, formSolidB[0]?.volume_fraction, formSolidMix[0]?.volume_fraction], unit: 'wt%' },
          { label: '组分A的体积比', values: [formStat[0]?.volume_ratio_a, formSolidB[0]?.volume_ratio_a, formSolidMix[0]?.volume_ratio_a], unit: 'v%' },
          { label: 'PVC', values: [formSolidA[0]?.pvc, formSolidB[0]?.pvc, formSolidMix[0]?.pvc], unit: 'v%' }
        ]
      }
    }
  ]
