...
const HomeFormPage: React.FC<Props> = ({ searchKeyword = '' }) => {
  ...
  const tableColumns: TableColumn[] = [
    { key: 'material_code', label: '原料代码' },
    { key: 'material_desc', label: '原料描述', width: '260px' },
    { key: 'test_quality', label: '测试质量' },
  ...
  // 关键字过滤
  const filteredFormDataA = React.useMemo(() => {
    const keyword = searchKeyword.trim().toLowerCase()
    if (!keyword) return formDataA as TableDataItem[]
    return (formDataA as TableDataItem[]).filter(
      (item) =>
        `${item.material_code ?? ''}`.toLowerCase().includes(keyword) ||
        `${item.material_desc ?? ''}`.toLowerCase().includes(keyword)
    )
  }, [formDataA, searchKeyword])

  const filteredFormDataB = React.useMemo(() => {
    const keyword = searchKeyword.trim().toLowerCase()
    if (!keyword) return formDataB as TableDataItem[]
    return (formDataB as TableDataItem[]).filter(
      (item) =>
        `${item.material_code ?? ''}`.toLowerCase().includes(keyword) ||
        `${item.material_desc ?? ''}`.toLowerCase().includes(keyword)
    )
  }, [formDataB, searchKeyword])
  ...
  React.useEffect(() => {
    const loadData = async () => {
      try {
        ...
        window.api.dataviz.getFormulaMaterials(2).then((rows: any[]) => {
          setFormDataA(rows.filter((r) => r.group_type === 'A'))
          setFormDataB(rows.filter((r) => r.group_type === 'B'))
        })

        window.api.dataviz.getFormulaStats(2).then((rows: any[]) => {
          setFormStat(rows)
        })
        window.api.dataviz.getFormulaSolids(2).then((rows: any[]) => {
  ...
  return (
    <FormContainer>
      ...
      <FormContent>
        <TablesContainer>
          {tableData.map((table) => (
            <FormTableComponent key={table.title} title={table.title} data={table.data} columns={tableColumns} />
          ))}
        </TablesContainer>
