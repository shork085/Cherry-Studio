...
const FormTableComponent: React.FC<FormTableComponentProps> = ({ title, data, columns }) => {
  // 渲染单元格内容
  const renderCellContent = (item: TableDataItem, key: string) => {
    switch (key) {
      case 'test_quality':
      case 'quality_score':
        return Number(item[key as keyof TableDataItem]).toFixed(2)
      case 'mix': {
        const mixValue = item[key as keyof TableDataItem]
...
const HomeFormPage: React.FC = () => {
  ...
  // 表格数据配置
  const tableData = [
    { title: '组分A', data: formDataA },
    { title: '组分B', data: formDataB }
  ]
