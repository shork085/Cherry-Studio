...
const HomeFormPage: React.FC<Props> = ({ searchKeyword = '', stoichRatio = 1 }) => {
  ...
  // 固含（当前仅使用 setter，因此丢弃 state 以避免未使用告警）
  ...
  const bottomFormConfig = [
    {
      title: '配方比重',
      items: [
        { label: '组分A的比重', value: Number(computedStats?.view.density_a).toFixed(2) ?? '-', unit: 'g/cm3' },
        { label: '组分B的比重', value: Number(computedStats?.view.density_b).toFixed(2) ?? '-', unit: 'g/cm3' },
        { label: '混合物的比重', value: Number(computedStats?.view.density_mix).toFixed(2) ?? '-', unit: 'g/cm3' }
      ]
    },
    {
      title: '配比',
      ...
      items: [
        {
          label: '组分A的质量比',
          value: Number(computedStats?.view.mass_ratio_a).toFixed(2) ?? '-',
          unit: 'wt%'
        },
        {
          label: '组分B的质量比',
          value: Number(computedStats?.view.mass_ratio_b).toFixed(2) ?? '-',
          unit: 'wt%'
        },
        {
          label: '组分A的体积比',
          value: Number(computedStats?.view.volume_ratio_a).toFixed(2) ?? '-',
          unit: 'v%'
        },
        {
          label: '组分B的体积比',
          value: Number(computedStats?.view.volume_ratio_b).toFixed(2) ?? '-',
          unit: 'v%'
        }
      ]
    },
        ...
        rows: [
          {
            label: '质量固含(wt%)',
            values: [
              Number(computedStats?.solids.A.mass_fraction).toFixed(2),
              Number(computedStats?.solids.B.mass_fraction).toFixed(2),
              Number(computedStats?.solids.MIX.mass_fraction).toFixed(2)
            ],
            unit: 'wt%'
          },
          {
            label: '体积固含(v%)',
            values: [
              Number(computedStats?.solids.A.volume_fraction).toFixed(2),
              Number(computedStats?.solids.B.volume_fraction).toFixed(2),
              Number(computedStats?.solids.MIX.volume_fraction).toFixed(2)
            ],
            unit: 'wt%'
          },
          {
        ...
        rows: [
          {
            label: 'reactive',
            values: [
              computedStats?.reactivity.A.reactive_binder_wt_set,
              computedStats?.reactivity.B.reactive_binder_wt_set,
              computedStats?.reactivity.MIX.reactive_binder_wt_set
            ]
          },
          {
            label: 'nonreactive',
            values: [
              computedStats?.reactivity.A.non_reactive_binder_wt_set,
              computedStats?.reactivity.B.non_reactive_binder_wt_set,
              computedStats?.reactivity.MIX.non_reactive_binder_wt_set
            ]
          },
          {
            label: 'total binder',
            values: [
              Number(
                (computedStats?.reactivity.A.reactive_binder_wt_set || 0) +
                  (computedStats?.reactivity.A.non_reactive_binder_wt_set || 0)
              ).toFixed(2),
              Number(
                (computedStats?.reactivity.B.reactive_binder_wt_set || 0) +
                  (computedStats?.reactivity.B.non_reactive_binder_wt_set || 0)
              ).toFixed(2),
              computedStats?.reactivity.MIX.total_binder_wt_set
            ]
          }
        ]
      }
    },
    {
      ...
      ratioData: {
        ...
        rows: [
          {
            label: 'reactive',
            value: computedStats?.reactivity.MIX.reactive_pct_in_binder,
            unit: '%'
          },
          {
            label: 'non reactive',
            value: computedStats?.reactivity.MIX.non_reactive_pct_in_binder,
            unit: '%'
          }
        ]
      }
              ...
              <BottomFormItem
                ...
                className={
                  config.isSmall
                    ...
                    : config.isSolidTable
                      ? 'solidTable'
                      : config.isReactiveTable
                        ? 'reactiveTable'
                        : config.isRatioTable
                          ? 'ratioTable'
                          : ''
                }>
                {!config.isSolidTable && !config.isReactiveTable && !config.isRatioTable && (
                ...
                {config.isSolidTable ? (
                  ...
                  <ReactiveTableContainer>
                    <ReactiveTableHeader>
                      <div></div>
                      {config.reactiveData.headers.map((header, index) => (
                        <div key={index}>{header}</div>
                      ))}
                    </ReactiveTableHeader>
                    {config.reactiveData.rows.map((row, rowIndex) => (
                      <ReactiveTableRow key={rowIndex}>
                        <div title={row.label}>{row.label}</div>
                        {row.values.map((value, colIndex) => (
                          <div key={colIndex} title={String(value ?? '-')}>
                            {value ?? '-'}
                  ...
                  <RatioTableContainer>
                    <RatioTableTitle>{config.ratioData.title}</RatioTableTitle>
                    {config.ratioData.rows.map((row, rowIndex) => (
                      <RatioTableRow key={rowIndex}>
                        <div title={row.label}>{row.label}</div>
                        <div title={String(row.value ?? '-')}>{row.value ?? '-'}</div>
                        <div title={row.unit}>{row.unit}</div>
                      </RatioTableRow>
                    ))}
                  </RatioTableContainer>
                ) : (
...
// 反应/非反应表格样式
const ReactiveTableContainer = styled.div`
  height: 100%;
  display: flex;
  flex-direction: column;
  background: #fff;
...
const ReactiveTableHeader = styled.div`
  display: grid;
  grid-template-columns: 1fr 1fr 1fr 1fr;
  height: 25%;
  background: #f5f5f5;
...
const ReactiveTableRow = styled.div`
  display: grid;
  grid-template-columns: 1fr 1fr 1fr 1fr;
  height: 25%;
  border-bottom: 1px solid #cfcfcf;
...
// ratio reactive / non reactive binder 表格样式
const RatioTableContainer = styled.div`
  height: 100%;
  display: flex;
  flex-direction: column;
  background: #fff;
...
const RatioTableTitle = styled.div`
  height: 33.33%;
  display: flex;
  align-items: center;
  justify-content: center;
...
const RatioTableRow = styled.div`
  display: grid;
