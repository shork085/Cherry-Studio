...
const HomeFormPage: React.FC<Props> = ({ searchKeyword = '' }) => {
  ...
  const loadFormulaData = async (formulaId: number) => {
    try {
      ...
      setFormSolidA(solids.filter((row: any) => row.group_type === 'A'))
      setFormSolidB(solids.filter((row: any) => row.group_type === 'B'))
      setFormSolidMix(solids.filter((row: any) => row.group_type === 'MIX'))
    } catch (error) {
  ...
  // 处理配方类型切换
  const handleFormulaChange = async (typeName: string) => {
    setActiveBtnName(typeName)
    try {
      const formulaId = await window.api.dataviz.getFormulaId(typeName)
      if (formulaId) {
