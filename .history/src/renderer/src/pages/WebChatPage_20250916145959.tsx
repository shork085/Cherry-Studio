    ...
    try {
      await safeSend(history, (token) => {
        if (token.startsWith('__REASONING__:')) {
          ...
          setMessages((prev) => prev) // 保持引用不变，仅更新思考文本
          setReasoningMap((prev) => {
            const idx = prevTokenTargetIndexRef.current
            return { ...prev, [idx]: (prev[idx] || '') + t }
          })
          return
        }
        setMessages((prev) => {
          ...
          {messages.map((m, i) => (
            <MessageContainer key={i} $self={m.role === 'user'}>
              {editingMessage === i ? (
                ...
                <>
                  {/* 思考过程（折叠），显示在助理泡泡上方 */}
                  {m.role === 'assistant' && (reasoningMap[i] || (loading && i === prevTokenTargetIndexRef.current)) && (
                    <Row $self={false}>
                      <ReasoningContainer>
                        <ReasoningHeader onClick={() => setReasoningCollapsed((prev) => ({ ...prev, [i]: !prev[i] }))}>
                          {loading && i === prevTokenTargetIndexRef.current && !reasoningMap[i]
                            ? '正在思考中…（点击可折叠）'
                            : reasoningCollapsed[i]
                            ? '思考过程（已折叠，点击展开）'
                            : '思考过程（点击折叠）'}
                        </ReasoningHeader>
