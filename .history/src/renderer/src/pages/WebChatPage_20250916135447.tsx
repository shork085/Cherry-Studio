...
const WebChatPage: React.FC = () => {
  ...
  const trySendOrRegenerate = async (params: { messages: ChatMessage[]; onToken: (token: string) => void }) => {
    // 这个函数封装调用 sendMessage, 捕获 400 / 403 错误，标记模型不可用
    try {
      ...
      //  fetch 失败或 backend 抛出的异常

      console.error('sendMessage error for model', selectedModel, err)
      // 假设 err.status 和 err.message 存在
      const status = err.status
      const message = err.message || ''
      const reason = status === 403 ? '权限不足' : status === 400 ? '参数不合法或模型不存在' : '未知错误'
      setUnusableModels((prev) => ({
        ...prev,
        [selectedModel]: reason + (message ? `: ${message}` : '')
      }))
      throw err
    }
  }
                ...
                <ModelDropdown>
                  {availableModels.map((model) => (
                    <ModelOption
                      ...
                      {/* 如果你能从 model data 拿到一个 name 或 description，也可以显示 */}
                      {/* <ModelDescription>来源: {model.owned_by || 'unknown'}</ModelDescription> */}
                    </ModelOption>
                  ))}
                </ModelDropdown>
              )}
