...
const WebChatPage: React.FC = () => {
  ...
  // 处理未带专用前缀的 token，识别特定格式来将"思考"内容分离
  ...
  const safeSend = async (
    ...
    try {
      ...
      // 如果是手动中止（暂停），跳过错误处理，直接抛出给上层以停止流
      const isAborted = err?.name === 'AbortError' || /abort/i.test(String(err?.message || ''))
      if (isAborted) {
        throw err
      }
      // 捕获错误状态码 + message
  ...
  // 用户发送信息时调用，将用户输入添加到消息列表，初始化一个新的空助理消息，然后调用 safeSend 来获取助理的回复（流式或一次性）。
  const onSend = async () => {
    ...
    try {
      ...
      const isAborted = e?.name === 'AbortError' || /abort/i.test(String(e?.message || ''))
      if (isAborted) {
        // 手动停止：不标红，不改写已有内容
        return
      }
...
const ReasoningCollapse = styled.div<{ $open: boolean }>`
  overflow: hidden;
  transition:
  ...
  transform: ${(p) => (p.$open ? 'translateY(0)' : 'translateY(-2px)')};
`
