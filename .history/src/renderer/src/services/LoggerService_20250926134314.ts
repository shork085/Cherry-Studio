...
const IS_DEV = IS_WORKER ? false : (window.electron as any)?.process?.env?.NODE_ENV === 'development'
...
class LoggerService {
  ...
  private constructor() {
    if (IS_DEV) {
      if (
        (window.electron as any)?.process?.env?.CSLOGGER_RENDERER_LEVEL &&
        Object.values(LEVEL).includes((window.electron as any)?.process?.env?.CSLOGGER_RENDERER_LEVEL as LogLevel)
      ) {
        this.envLevel = (window.electron as any)?.process?.env?.CSLOGGER_RENDERER_LEVEL as LogLevel
      ...
      if ((window.electron as any)?.process?.env?.CSLOGGER_RENDERER_SHOW_MODULES) {
        const showModules = (window.electron as any)?.process?.env?.CSLOGGER_RENDERER_SHOW_MODULES.split(',')
