...
// TODO：考虑拆开 入口函数，用户发送消息后决定是否调用外部工具
async function fetchExternalTool(
  ...
  const extract = async (): Promise<ExtractResults | undefined> => {
    ...
    // 根据配置决定是否需要提取
    const needWebExtract = shouldWebSearch //是否要为Web搜索提取关键词
    const needKnowledgeExtract = hasKnowledgeBase && knowledgeRecognition === 'on' //是否要为知识库搜索提取关键词
  ...
  const searchTheWeb = async (
    ...
    // Add check for which assistant.model before using it
    if (!assistant.model) {
    ...
    try {
      ...
      const webSearchResponse = await WebSearchService.processWebsearch(
        safeWebSearchProvider!,
        extractResults, //关键词
        lastUserMessage.id //标记哪条信息触发搜索
      )
      return {
        results: webSearchResponse,
        source: WebSearchSource.WEBSEARCH //标记为Web搜索
      }
    } catch (error) {
  ...
  try {
    ...
    let webSearchResponseFromSearch: WebSearchResponse | undefined //来自Web搜索的结果
    let knowledgeReferencesFromSearch: KnowledgeReference[] | undefined //来自知识库参考资料
    let memorySearchReferences: MemoryItem[] | undefined //来自记忆搜索的结果
    ...
    if (showListTools) {
      try {
        ...
        // 获取所有启用的MCP服务器,为每个服务器创建一个异步服务,返回工具列表MCPTool[]
...
export async function fetchChatCompletion({
  ...
  // Web 环境使用简化的 WebChatBackend
  if (typeof window !== 'undefined' && !window.navigator.userAgent.includes('Electron')) {
    const { WebChatBackend } = await import('./webchat/ChatBackend')
    ...
    try {
      const result = await backend.sendMessage({
        ...
        onToken: (token) => {
          onChunkReceived({ type: 'token', content: token })
        }
      })

      onChunkReceived({ type: 'done', content: result.content })
      return
    } catch (error) {
      logger.error('Web chat completion failed:', error)
      onChunkReceived({ type: 'error', content: 'Web 环境聊天服务暂时不可用' })
      return
    }
  }
  ...
  // 内部先调用fetchExternalTool获取外部搜索结果,再把用户消息交给AI
