...
const api = {
  ...
  dataviz: {
    initDb: (config: { host: string; user: string; password: string; database: string; port?: number }) =>
      ipcRenderer.invoke(IpcChannel.Dataviz_InitDb, config),
    getFormData: (): Promise<{ A: any[]; B: any[] }> => ipcRenderer.invoke(IpcChannel.Dataviz_GetFormData)
  },
  contextBridge.exposeInMainWorld('dataviz',
    {
      getFormData: (formulaId:number) =>
       ipcRenderer.invoke(IpcChannel.Dataviz_GetFormData,formulaId)
    }
  )
}
