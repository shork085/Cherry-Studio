...
export function registerIpc(mainWindow: BrowserWindow, app: Electron.App) {
  ...
  ipcMain.handle(
    ...
    async (_, config: { host: string; user: string; password: string; database: string; port?: number }) => {
      await mySQLService.init(config)
    }
  )

  ipcMain.handle(IpcChannel.Dataviz_GetFormData, async (_event, formulaId: number) => {
    return await mySQLService.getFormData(formulaId)
  })
}
