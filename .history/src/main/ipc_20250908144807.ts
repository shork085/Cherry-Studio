...
export function registerIpc(mainWindow: BrowserWindow, app: Electron.App) {
  ...
  // MySQL
  ipcMain.handle(IpcChannel.Mysql_SetConfig, async (_e, config) => {
    await mysqlService.setConfig(config)
  })
  ipcMain.handle(IpcChannel.Mysql_QueryForm, async (_e, { tableA, tableB }) => {
    return await mysqlService.fetchFormData(tableA, tableB)
  })
}
