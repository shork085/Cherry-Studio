...
export class MySQLService {
  ...
  public async getMaterialVersionByRowId(id: number): Promise<number | null> {
    try {
      const rows = await this.query<MaterialVersionRow>('SELECT version FROM formula_materials WHERE id=?', [id])
      return rows.length > 0 ? rows[0].version : null
    } catch (error) {
      console.error('获取原料当前版本号失败:', error)
      return null
  ...
  public async updateMaterialCode(
    ...
    try {
      ...
      if (err.code === '45001') {
        throw new Error('新原料代码不存在')
      } else if (err.code === '45002') {
