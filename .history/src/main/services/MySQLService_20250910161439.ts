...
export class MySQLService {
  ...
  public async calculateQualityScore(formulaId: number) {
    const sql =
      'SELECT fm.id,fm.formula_id,fm.group_type,fm.material_code,fm.test_quality,' +
      'ROUND(fm.test_quality / t.total_quality * 100, 2) AS calc_quality_fraction' +
      ...
      ') t ON fm.formula_id = t.formula_id AND fm.group_type = t.group_type' +
      'WHERE fm.formula_id=?'
