...
export class MySQLService {
  ...
  // 计算质量分数
  public async calculateQualityScore(formulaId: number, groupType: string) {
    const sql =
      'SELECT fm.formula_id, fm.group_type, fm.test_quality, t.total_quality,' +
      'ROUND(fm.test_quality / t.total_quality * 100, 2) AS calc_quality' +
      'FROM formula_materials fm' +
      'JOIN (SELECT formula_id, group_type, SUM(test_quality) AS total_quality' +
      'FROM formula_materials' +
      'GROUP BY formula_id, group_type) t' +
      'ON fm.formula_id = t.formula_id AND fm.group_type = t.group_type' +
      'WHERE fm.formula_id = ?' +
      'AND fm.group_type = ?'
    return this.query(sql, [formulaId], [groupType])
  }
}
