...
export function registerIpc(mainWindow: BrowserWindow, app: Electron.App) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      await mySQLService.init(config)
    }
  )

  ipcMain.handle(IpcChannel.Dataviz_GetFormulas, async () => {
    return await mySQLService.getFormulas()
  })

  ipcMain.handle(IpcChannel.Dataviz_GetFormulaMaterials, async (_event, formulaId: number) => {
    return await mySQLService.getFormulaMaterials(formulaId)
  })
  ipcMain.handle(IpcChannel.Dataviz_GetFormulaStats, async (_event, formulaId: number) => {
    return await mySQLService.getFormulaStats(formulaId)
  })
  ipcMain.handle(IpcChannel.Dataviz_GetFormulaSolids, async (_event, formulaId: number) => {
    return await mySQLService.getFormulaSolids(formulaId)
  })
  ipcMain.handle(IpcChannel.Dataviz_GetFormulaIdByType, async (_event, formulaName: string) => {
    return await mySQLService.getFormulaIdByType(formulaName)
  })
  ipcMain.handle(
    IpcChannel.Dataviz_GetMaterialVersion,
    async (_event, formulaId: number, materialId: number, groupType: string) => {
      return await mySQLService.getMaterialVersion(formulaId, materialId, groupType)
    }
  )
  ipcMain.handle(IpcChannel.Dataviz_GetMaterialVersionByRowId, async (_event, id: number) => {
    return await mySQLService.getMaterialVersionByRowId(id)
  })
  ipcMain.handle(
    IpcChannel.Dataviz_UpdateTestQuality,
    async (
      _event,
      id: number,
      // materialId: number,
      ...
      return await mySQLService.updateMaterialTestQuality(
        id,
        // materialId,
        // groupType,
        newTestQuality,
  ...
  ipcMain.handle(
    IpcChannel.Dataviz_UpdateMaterialCode,
    async (
      _event,
      id: number,
      ...
      return await mySQLService.updateMaterialCode(
        id,
        // oldMaterialCode,
        newMaterialCode,
        // groupType,
